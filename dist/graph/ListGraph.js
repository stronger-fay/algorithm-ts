...
class ListGraph extends Graph_1.Graph {
    ...
    shortestPaths() {
        ...
        for (let edge of this.edges) {
            ...
            let pathInfo = new Graph_1.PathInfo(edge.weight);
        ...
        this.vertices.forEach((vertex2, v2) => {
            this.vertices.forEach((vertex1, v1) => {
                this.vertices.forEach((vertex3, v3) => {
                    var _a;
                    if (_.isEqual(v1, v2) || _.isEqual(v2, v3) || _.isEqual(v1, v3))
                        return;
                    // v1 -> v2
                    ...
                    if (path13 !== undefined && this.weightManager.compare(newWeight, path13.weight) >= 0)
                        return;
                    if (path13 === undefined) {
                        path13 = new Graph_1.PathInfo();
                        (_a = paths.get(v1)) === null || _a === void 0 ? void 0 : _a.set(v3, path13);
                    }
                    else {
                    ...
                    for (let i = 0; i < path12.edgeInfos.length(); i++) {
                        const edgeInfo = path12.edgeInfos.get(i);
                        path13.edgeInfos.add(edgeInfo);
                    }
                    for (let i = 0; i < path23.edgeInfos.length(); i++) {
                        const edgeInfo = path23.edgeInfos.get(i);
                        path13.edgeInfos.add(edgeInfo);
                    }
                });
            });
